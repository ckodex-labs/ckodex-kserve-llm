...
5. **Hosted signal:** the latest scheduled Nightly KIND Chaos run
   ([31771937532](https://github.com/ckodex-labs/ckodex-kserve-llm/actions/runs/31771937532))
   ran at the old published head and failed immediately after applying the sample
   workload because the E2E script waited on a pod selector before the controller
   had created a pod. This is tracked as a release-blocking acceptance-harness
   defect, not as evidence that inference itself failed; the fixed head still
   needs a hosted rerun.
...
| BETA-P0-012 | The local prerequisite script did not install the external Gateway API Inference Extension CRDs required by the EPP path. | The scheduler could not create or observe InferencePool resources in a fresh cluster. | Fixed locally: the official Gateway API Inference Extension manifest is installed by the default prerequisite path. | Hosted fresh-cluster prerequisite and runtime rerun |
...
| BETA-P0-015 | The local KIND configuration inherited a moving node-image default and the host exposed only a 1,024-file-descriptor node limit, causing systemd to fail before Kubernetes boot. | The default fresh-cluster proof could fail before cert-manager, CRDs, or the operator were evaluated and provide no actionable diagnosis. | Fixed locally: pin the disposable acceptance environment to `kindest/node:v1.35.0` and fail preflight below 65,536 descriptors; revisit only with an intentional Kubernetes-version qualification. | Fresh KIND rerun on a host meeting the preflight and hosted environment parity |
