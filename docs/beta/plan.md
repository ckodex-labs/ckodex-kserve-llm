...
| BETA-P0-014 | RC6 chart metadata was `0.18.0-rc.6` while default operator and initializer values still referenced `v0.18.0-beta.8`. | Installing the RC chart without overrides could pull a different release unit than the chart and release notes describe. | Fixed and hosted-verified in RC7: empty image tags resolve from `Chart.appVersion`, the release workflow preserves the leading `v` in packaged `appVersion`, and the packaged chart passed render and anonymous artifact acceptance in [run 33457020052](https://github.com/ckodex-labs/ckodex-kserve-llm/actions/runs/33457020052). | Closed for the RC7 release unit |
