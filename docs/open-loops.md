...
- **Context:** Both workflows migrated (2026-06-14):
  - `ci.yml`: replaced `go run ./ci/main.go` with Dagger hosted fast gate (`dagger call all --source=.`) and vulnerability scan. Coverage remains available via `dagger call coverage --source=.` outside the hosted fast path.
