...
func (r *LocalModelCacheReconciler) warmupPodSpec(lmc *servingv1alpha2.LocalModelCache, pvcName, nodeName string) corev1.PodSpec {
	...
	podSpec := corev1.PodSpec{
		...
		TerminationGracePeriodSeconds: ptr.To(int64(ASRTerminationGracePeriod)),
		...
		Containers: []corev1.Container{warmupContainer(lmc)},
...
func warmupContainer(lmc *servingv1alpha2.LocalModelCache) corev1.Container {
	nonRoot := true
	nonRootUID := int64(65532)
	nonRootGID := int64(65532)
	...
	return corev1.Container{
		Name: "warmup", Image: CKodexStorageInitializerImage, ImagePullPolicy: corev1.PullIfNotPresent,
...
func warmupResources() corev1.ResourceRequirements {
	cpu := resource.MustParse(DefaultCacheCPURequest)
	memory := resource.MustParse(DefaultCacheMemoryRequest)
