...
// ReconcileNetworkPolicy creates a comprehensive set of egress and ingress policies.
// It enforces a "Secure-by-Default" posture with:
//  1. deny-all-ingress  — blocks all ingress by default
...
//  4. egress-lockdown    — strict egress control permitting only DNS, Mesh, and SPIRE
func (r *NetworkPolicyReconciler) ReconcileNetworkPolicy(ctx context.Context, llmSvc *servingv1alpha2.LLMInferenceService) error {
	...
	policies := []*networkingv1.NetworkPolicy{denyIngress, allowGateway, egressLockdown, allowIntra}
